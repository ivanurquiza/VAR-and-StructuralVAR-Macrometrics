...
datos <- read_excel("datos/rawdb.xlsx", sheet = 1, skip = 5)
...
orden_var <- c("actividad_mex_sa_log", "ipc_mex_sa_log", "tpm_mex", "embi_mex", "tcn_mex_log")

Y <- mts_mexico[, orden_var]
...
irf_one_chi <- irf(SVAR, response = "actividad_mex_sa_log", impulse = "tpm_mex", 
               n.ahead = 10, ortho = TRUE, boot = TRUE)

#---Plots IRFs---
irf_matrix <- irf_one_chi$irf$tpm_mex  # matriz de respuestas al impulso tpm_chi

# Construir dataframe
irf_df_chi <- data.frame(
  period = 0:(nrow(irf_matrix)-1),  # horizontes
  response = irf_matrix[, "actividad_mex_sa_log"],  # columna de respuesta
  lower = irf_one_chi$Lower$tpm_mex[, "actividad_mex_sa_log"],
  upper = irf_one_chi$Upper$tpm_mex[, "actividad_mex_sa_log"]
)
...
ggplot(irf_df_chi, aes(x = period, y = response)) +
  ...
  labs(
    title = "Respuesta Impulso-Respuesta (IRF) para Mexico",
...
# Cambio nombres
shocks <- c("ACTIVIDAD", "IPC", "TPM_MEX", "EMBI", "TCN")
...
ggplot(irf_df, aes(x = periodo, y = valor)) +
  ...
  labs(
    title = "IRF: Respuesta de las variables a un shock en TPM_MEX",
...
dimnames(FEVD)[[2]] <- c("ACTIVIDAD", "IPC", "TPM_MEX", "EMBI", "TCN")
...
irf_df_boot <- as.data.frame.table(IRF.boot$pe, responseName = "valor") %>%
  ...
  filter(shock == "TPM_MEX")  # solo respuestas al shock de política monetaria
...
ggplot(irf_df_boot, aes(x = periodo, y = valor)) +
  ...
  facet_wrap(~ variable, scales = "free_y", ncol = 2) +
  labs(
    title = "IRF con Bootstrap: Respuesta de las variables a un shock en TPM_MEX",
    x = "Horizonte (meses)", y = "Respuesta"
  ) +
  theme_minimal(base_family = "Lato") +
  theme(
...
ggplot(irf_df_acc, aes(x = periodo, y = valor)) +
  ...
  facet_wrap(~ variable, scales = "free_y", ncol = 2) +
  labs(
    title = "IRF acumulada bootstrap: Respuesta a un shock en TPM_MEX",
...
#Mexico con variables internacionales---------------------------------------
...
orden_var_int_chi <- c("actividad_global", "ipc_usa_sa_log", "shadow_ffr" ,"ebp",
                   ...
                   "tot_mex_sa_log","actividad_mex_sa_log", 
                   
                   "ipc_mex_sa_log", "tpm_mex", "embi_mex", "tcn_mex_log")


mts_chile_ <-mts_mexico[1:(nrow(mts_inter)), ]
...
irf_one_chi_int <- irf(SVAR_inter_chi, response = "actividad_mex_sa_log", impulse = "tpm_mex", 
                   n.ahead = 10, ortho = TRUE, boot = TRUE)

#---Plots IRFs---
irf_matrix_chi_int <- irf_one_chi_int$irf$tpm_mex  # matriz de respuestas al impulso tpm_chi

# Construir dataframe
irf_df_chi_int <- data.frame(
  period = 0:(nrow(irf_matrix_chi_int)-1),  # horizontes
  response = irf_matrix_chi_int[, "actividad_mex_sa_log"],  # columna de respuesta
  lower = irf_one_chi_int$Lower$tpm_mex[, "actividad_mex_sa_log"],
  upper = irf_one_chi_int$Upper$tpm_mex[, "actividad_mex_sa_log"]
)
...
ggplot(irf_df_chi_int, aes(x = period, y = response)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), 
              fill = "#006400", alpha = 0.15) +
  geom_line(color = "#006400", size = 1.4) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50", size = 0.8) +
  labs(
    title = "Función Impulso-Respuesta (IRF) para Mexico con variables internacionales",
    subtitle = "Respuesta de la tasa de cambio de la actividad a un shock en la tasa de política monetaria",
    x = "Horizonte (meses)", y = "Cambio en la actividad"
  ) +
  theme_minimal(base_family = "lato") +
...
ggplot(irf_df_boot, aes(x = periodo, y = valor)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  geom_ribbon(aes(ymin = lb, ymax = ub), fill = "#006400", alpha = 0.2) +
  geom_line(color = "#006400", size = 1.1) +
  facet_wrap(~ variable, scales = "free_y", ncol = 2) +
  labs(
    title = "IRF con Bootstrap: Respuesta de las variables a un shock en TPM_MEX",
    x = "Horizonte (meses)", y = "Respuesta"
...
ggplot(irf_df_acc, aes(x = periodo, y = valor)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray60", size = 0.8) +
  geom_ribbon(aes(ymin = lb, ymax = ub), fill = "#74AA9C", alpha = 0.2) +
  geom_line(color = "#74AA9C", size = 1.1) +
  facet_wrap(~ variable, scales = "free_y", ncol = 2) +
  labs(
    title = "IRF acumulada: Respuesta a un shock en TPM_MEX",
    x = "Horizonte (meses)", y = "Respuesta acumulada"
  ) +
  theme_minimal(base_family = "lato") +
  theme(
